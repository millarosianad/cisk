...
class Trans_model extends CI_Model
{
    ...
    private function getHarga($kode=null,$user=null)
    {
        ...
        $sql='
        select exclude,level,diskon,id,username 
        from mpm.user where id='.$client;
        ...
        switch($level)
        {
            case 4:
                ...
                //cek apakah DP pulau jawa atau tidak
                $this->db->where('kode_comp = '.'"'.$username.'"');
                $this->db->where('status','1');
                $query = $this->db->get('mpm.tbl_tabcomp',1);
                ...
                if ($jawa == '1') {
                    ...
                    $sql="select h_luarjawa-(h_luarjawa*d_luarjawa/100) as harga from prod_detail where kodeprod='".$kode."' and tgl=(select max(tgl) from prod_detail where kodeprod='".$kode."')";
                }
                print_r($sql);
            ...
            case 5:
                if ($id=='233') {
                    ...
                    $sql="
                    select  h_dpbatam-(h_dpbatam*".$diskon."/100) as harga 
                    from    mpm.prod_detail 
                    where   kodeprod='".$kode."' and 
                            tgl=(
                                select  max(tgl) 
                                from    mpm.prod_detail 
                                where   kodeprod='".$kode."'
                    ...
                    $sql="
                    select  h_pbf-(h_pbf*".$diskon."/100) as harga 
                    from    mpm.prod_detail 
                    where   kodeprod='".$kode."' and 
                            tgl=(
                                select  max(tgl) 
    ...
    public function getCustInfo()
    {
        ...
        $sql="select company,npwp,address,email,b.kode_alamat from mpm.user a
            left JOIN mpm.t_alamat b on a.username = b.username WHERE id=?";
        $query=$this->db->query($sql,array($id));
        return $query->row();
    }

    public function getCustInfoRetur($id=null)
    {
        //$id=$this->session->userdata('id');
        $sql='select company,nama_wp,npwp,address,email,alamat_wp,nama_retur,npwp_retur from mpm.user where id=?';
        $query=$this->db->query($sql,array($id));
        return $query->row();
    }
    ...
    {
        //$id=$this->session->userdata('id');
        $sql='select company,nama_wp,npwp,address,email,alamat_wp, b.kode_alamat from mpm.user a
            left JOIN mpm.t_alamat b on a.username = b.username where id=?';
        $query=$this->db->query($sql,array($id));
        return $query->row();
    }
    ...
    {
        $sql="
                select a.banyak_karton, a.berat, sum(a.banyak_karton*a.berat) as tot_berat,a.volume, sum(a.banyak_karton*a.volume) as tot_volume
                from mpm.po_temp a
                where userid=$id 
                order by kodeprod
            ";
    ...
    public function list_product_supp($supp)
    {
        ...
        if($this->session->userdata('level')==7)
        {
            return $this->db->query('select kodeprod, namaprod,isisatuan,odrunit from mpm.tabprod  where active=1 and produksi = 1  and permen=1 and supp="'.$supp.'" or (KODEPROD = "'.$prod_tambahan.'")order by namaprod');
        }
        else 
        {
            return $this->db->query('select kodeprod, namaprod,isisatuan,odrunit from mpm.tabprod  where active=1 and produksi = 1 and supp="'.$supp.'" order by namaprod');
        }
    }
    ...
    public function list_product_supp_retur($supp)
    {
            ...
            return $this->db->query('
                select a.kodeprod,a.NAMAPROD as namaprod,b.NAMAPROD as old
                from mpm.tabprod a LEFT JOIN mpm.tabprod_backup5nov2018 b on a.KODEPROD = b.KODEPROD
                where a.supp ="'.$supp.'"
                ORDER BY b.namaprod
                ');

            $query = 'select kodeprod, namaprod,isisatuan from mpm.tabprod  where supp="'.$supp.'" order by namaprod';
    ...
    {
        return $this->db->query('
            select	a.supp,a.kodeprod,a.namaprod,a.kode_prc,a.grupprod, a.grup, a.isisatuan,b.h_dp, a.berat, a.volume 
            ...
            case 4:
                ...
                where   kodeprod="'.$kodeprod.'" and 
                        tgl=(
                            select max(tgl) 
                            from    mpm.prod_detail 
                            where   kodeprod="'.$kodeprod.'" and tgl<="'.$tgl.'"
                            )';
    ...
    {   
        switch($state)
        {
            ...
            {
                $this->email_config_piutang();
                $this->email->from('suriana@muliaputramandiri.com', "PT. Mulia Putra Mandiri");
                    
                $this->email->to($this->getEmailFinance(substr($id,1)));                    
                //$this->email->to("suffy.yanuar@gmail.com");
                
                $list = array('nanita@muliaputramandiri.com','hwiryanto@muliaputramandiri.com','herman.oscar@muliaputramandiri.com');
                //$list = 'suffy.yanuar@gmail.com';
                $this->email->cc($list);
...
order by a.group_descr
            ...
            case 'download':
            {
                ...
                switch($this->input->post('format'))
                {
                    ...
                    case 9:
                        {
                            ...
                            $tahun=substr($this->input->post('tanggal'),0,4);
                            delete_files('./assets/faktur');
                            $sql='select id,nodo_beli,supp from mpm.trans where supp=005 and month(tglbuat)=? and deleted=0 and year(tglbuat)=?' ;
                            $query=$this->db->query($sql,array($bulan,$tahun));
                    ...
                    case 8:
                    {
                        ...
                        echo "<pre>";
                        echo "<h3>Untuk sementara tidak bisa digunakan terlebih dahulu. Sedang dilakukan pengecekan.</h3>";
                        echo "bulan : ".$bulan."<br>";
                        echo "tahun : ".$tahun."<br>";
                        print_r($sql);
                        print_r($query);
                        ...
                        if($query->num_rows() > 0)                        
                        {
                            echo "tampil jika num_rows > 0<br><br>";
                            ...
                            foreach ($query->result() as $value)
                            {
                                echo "ini muncul di foreach<br>";
                                
                                 $PHPJasperXML = new PHPJasperXML();
                                 $PHPJasperXML->debugsql=false;
                                 $PHPJasperXML->arrayParameter=array('nopesan'=>$value->id);
                                 //echo $value->id."<br>";  
                                 echo $value->nodo_beli."<br>";                               
                        ...
                        {
                            //echo "muncul jika num_rows false";                         
                            return false;                            
                        }
                    ...
                    case 10:
                        {
                            ...
                            $tahun=substr($this->input->post('tanggal'),0,4);
                            delete_files('./assets/faktur');
                            $sql='select id,nodo_beli,supp from mpm.trans where supp=001 and month(tglbuat)=? and deleted=0 and year(tglbuat)=?' ;
                            $query=$this->db->query($sql,array($bulan,$tahun));
    ...
    public function repl($state=null,$key=0,$id='',$init='')
    {
        switch($state)
        {
            ...
            case 'delete_temp':
            {
                ...
                $sql2='delete from mpm.replresult_temp where created_by=?';
            ...
            case 'show_temp':
            {
                ...
                //menampilkan kode_comp berdasarkan customer id
                $cari_kode_comp = "
                    select username from mpm.user where id = ".$customerid."
                ";
                $proses_cari_kode_comp = $this->db->query($cari_kode_comp);
                $row=$proses_cari_kode_comp->row();

                $kode_comp = "'".$row->username."'";
                //echo "<hr>kode_comp : ".$kode_comp."<br>";
                
                $cari_group_repl= "
                    select group_repl,status_group_repl,jawa from mpm.tbl_tabcomp where kode_comp =".$kode_comp."
                ";
                //echo "<br><hr> cari_group_repl : ".$cari_group_repl."<br><hr>";
                ...
                if ($group_repl == '') { //JIKA DP TIDAK PUNYA GROUP / NORMAL
                    ...
                    $selectfi="
                    select  if(kodeprod='010020','010074',
                            IF(kodeprod='010057','010075',
                            IF(kodeprod='010006','010082',
                            IF(kodeprod='010007','010082',
                            IF(kodeprod='010008','010083',
                            IF(kodeprod='010009','010083',                            
                            IF(kodeprod='010058','010076',kodeprod)))))))kodeprod,
                    ...
                    for($i=2;$i<$counter;$i++)
                    {
                        $selectfi.="
                            ...
                            where   nocab=".'"'.$dp.'"'." and kode_type<>'TD' and bulan =".date('m',strtotime($date[$i]))."
                            group by kodeprod       
                        ";
                    }
                    
                    //echo "<pre>";
                    //echo "<hr> selectfi : ".$selectfi."<br><hr><br>";
                    //echo "</pre>";
                    ...
                    //echo "<pre>";
                    //echo "<hr> selectri : ".$selectri."<br><hr><br>";
                    //echo "</pre>";
                    /*
                        1. floor = memotong nilai dari suatu bilangan yang ada di belakang koma, ex : 123.6789 menjadi 123
                        2. abs = membuat nilai absolute / positif, ex : -100 menjadi 100
                        3. ceiling = membulatkan nilai, ex : 123.6789 menjadi 124
                        4. cast = sama dengan fungsi convert, yaitu merubah tipe data dari string ke integer
                    */

                    //perbedaan harga berdasarkan jawa dan luar jawa
                    $row=$this->getCustInfo2($customerid);
                    if ($jawa == '1')
                    {
                        $sql="
                            insert into mpm.replresult
                            select  '".$customerid."',
                                    '".$row->company."',
                                    '".$row->npwp."',
                                    '".$row->email."',
                                    ...
                                    karton,
                                    pesan,
                                    (pesan*karton) as unit,
                                    '".$tanggal."',
                                    '".date('Y-m-d H:i:s')."',
                                    ".$userid." 
                            from
                            (
                                    select  if(kodeprod='010020','010074',IF(kodeprod='010057','010075',IF(kodeprod='010058','010076',kodeprod))) kodeprod,
                                            b.namaprod,
                                            b.supp,
                                            kode_prc,
                                            sl,
                                            ...
                                            delta,
                                            karton as karton,
                                            if(delta<0,if((abs(delta)/karton)-floor(abs(delta)/karton)>=0.2,ceiling(abs(delta)/karton),floor(abs(delta)/karton)),0) as pesan 
                                    from
                                    (
                                        select  kodeprod,
                                                namaprod,
                                                ...
                                                rata,
                                                stok,
                                                cast((rata/30*sl) as unsigned) as stdsl,(stok-cast(rata/30*sl as unsigned)) as delta
                                        from
                                        (
                                            select  if(kodeprod='010020','010074',IF(kodeprod='010057','010075',IF(kodeprod='010058','010076',kodeprod))) kodeprod,
                                                    namaprod,
                                                    sl,
                                                    sum((Saldoawal+masuk_pbk+masuk_supp+retur_sal+Bpinjam+kvretur+retur_depo+tukar_msk+msk_gd_pst) - (sales+kvbeli+pinjam+retur_supp+minta_depo+tukar_klr+klr_gd_pst) )as stok
                                            from    data".$year.".st a 
                                                    inner join
                                                    (
                                                        select  kodeprod,
                                            ...
                                            where   nocab=".'"'.$dp.'"'." and
                                                    ...
                                                    (
                                                        ...
                                                        from    data".$year.".st 
                                                        where   nocab=".'"'.$dp.'"'."
                                                    ) and kode_gdg='PST' and sl<>0 
                                            group by kodeprod 
                                            ORDER BY KODEPROD
                                        )a  left join 
                                            (
                                                select  kodeprod,
                                                        cast(sum(average)/".$cycle." as unsigned) as rata 
                                                from
                                                (
                                                        ".$selectfi."                        
                                                        union all                        
                            ...
                            order by kodeprod
                                ...
                                (
                                    select  kodeprod,
                                            (a.h_dp-(a.h_dp*d_dp/100)) as harga 
                                    from    mpm.prod_detail a 
                                    where   tgl=
                                            (
                                                select  max(tgl) 
                        ...
                        $sql="
                            ...
                            order by kodeprod
                                ...
                                (
                                    select  kodeprod,
                                            (a.h_luarjawa-(a.h_luarjawa*d_luarjawa/100)) as harga 
                                    from    mpm.prod_detail a 
                                    where   tgl=
                                            (
                                                select  max(tgl) 
                ...
                /* proses gabungan pesan 010074 dan 700006 */
                    
                    $cek_sub= "
                        select  sub 
                        from    mpm.tabcomp where kode_comp =".$kode_comp."
                    ";
                
                $proses_cek_sub = $this->db->query($cek_sub);
                $row=$proses_cek_sub->row();

                $sub = $row->sub;
                echo "nilai sub : ".$sub."<br>";

                if ($sub == '98' || $sub == '91' || $sub == '89' || $sub == '27' || $sub == '18' || $sub == '46')
                { //Untuk DP di Pulau Jawa
                    
                    /*cek jumlah pesan 010074*/
                    $this->db->where('kodeprod = 010074');
                    $this->db->where('customerid = '.'"'.$customerid.'"');
                    $query = $this->db->get('mpm.replresult');
                    foreach ($query->result() as $row) {
                        $pesan_010074 = $row->pesan;
                        echo "pesan_010074 : ".$pesan_010074."<br>";
                        //return $wilayah_nocab;
                    }
                /*end cek jumlah pesan 010074*/

                /*cek jumlah pesan 700006*/
                    $this->db->where('kodeprod = 700006');
                    $this->db->where('customerid = '.'"'.$customerid.'"');
                    $query = $this->db->get('mpm.replresult');
                    foreach ($query->result() as $row) {
                        $pesan_700006 = $row->pesan;
                        //echo "pesan_700006 : ".$pesan_700006."<br>";
                        //return $wilayah_nocab;
                    }
                /*end cek jumlah pesan 700006*/

                /*menambahkan pesanan 010074 dan 700006*/
                    $total_pesanan = $pesan_700006 + $pesan_010074;
                    //echo "total pesanan 700006 : ".$total_pesanan;
                    $total_unit = $total_pesanan * 360;
                    //echo "total unit 700006 : ".$total_unit;
                    
                /*end menambahkan pesanan 010074 dan 700006*/

                /*mengupdate pesanan dari 700006*/
                $data = array(
                       'pesan' => $total_pesanan,
                       'unit' => $total_unit
                );
                ...
                $this->db->where('kodeprod = 700006');
                $this->db->where('customerid = '.'"'.$customerid.'"');
                $this->db->update('mpm.replresult', $data); 
                /*end mengupdate pesanan dari 700006*/

                /*mengupdate pesanan dari 010074*/
                $data = array(
                       'pesan' => $total_pesanan, 
                       'unit' => $total_unit
                );
                ...
                $this->db->where('customerid = '.'"'.$customerid.'"');
                $this->db->update('mpm.replresult', $data); 
                /*end mengupdate pesanan dari 010074*/

                }else{
                    //echo "yyy";
                }

                /* end proses gabungan pesan 010074 dan 700006 */
               

                    // **************** BATAS IF REPL TIDAK PUNYA GROUP
                } else { //JIKA DP PUNYA GROUP
                    
                    //*mencari dp lain yang satu group
                $cari_dp_lain= "
                    select kode_comp,naper from mpm.tbl_tabcomp where group_repl =".'"'.$group_repl.'"'.
                    " group by naper ORDER BY status_group_repl desc";
                ...
                {                   
                    ...
                    */
                    ...
                    {
                        $sql="
                        insert into mpm.replresult_temp
                        select  '".$customerid."',
                                '".$row->company."',
                                '".$row->npwp."',
                                '".$row->email."',
                ...
                //*mencari dp lain yang termasuk group replineshment
                $proses_repl= "
                        insert into mpm.replresult
                        select  customerid, company, npwp, email,
                        alamat, nocab, supp, kodeprod, namaprod,
                        kode_prc, sl, avg, stok, stdsl, delta, harga,
                        karton, SUM(pesan) pesan, SUM(unit) unit, tglpesan,
                        created, created_by             
                        from mpm.replresult_temp
                        where created = ".'"'.$tgl_input.'"'."
                        GROUP BY kodeprod
                        ORDER BY kodeprod
                    ";

                $query = $this->db->query($proses_repl);
            ...
            case 'list_upload':
            {
                ...
                '   select * from (
                    ...
                    ) a inner join (select nocab,nama_comp from mpm.tabcomp) b using(nocab) where userid like "'.$id.'" ORDER BY id desc';
    ...
    public function spk($state='show_supp',$key=null,$id=null)
    {
        switch(strtolower($state))
        {
            ...
            {
                $id=$this->session->userdata('id');
                $sql="delete from mpm.po_temp where userid=$id";
                $query=$this->db->query($sql,array($id));
                return $this->db->query($sql);
            }break;
            case 'show_temp':
                {
                    ...
                    if($query->num_rows() > 0)
                    {
                        ...
                        $this->table->set_heading('CODE','PRODUCT', 'PRC CODE','AMOUNT','BERAT','VOLUME','DELETE');
                        foreach ($query->result() as $value)
                        {
                            $js=array('onclick'=>"return confirm('Are you sure you want to delete?')");
                            $this->table->add_row(
                                    ...
                                    ,($value->berat*$value->banyak_karton) 
                                    ,number_format(($value->volume*$value->banyak_karton),3)
                ...
                case 'show_temp_total':
                {
                    $id=$this->session->userdata('id');
                    $supp = $this->session->userdata('supplier');
                    $sql1=  "
                                select a.banyak_karton, a.berat, sum(a.banyak_karton*a.berat) as tot_berat,a.volume, sum(a.banyak_karton*a.volume) as tot_volume, concat(format(sum(a.banyak_karton*if(a.berat is null, 'null', a.berat)),0),'&nbsp;kg','&nbsp;&nbsp; | &nbsp;&nbsp;',format(sum(a.banyak_karton*if(a.volume is null, 'null', a.volume)),3),'&nbsp;m3') as x
                                from mpm.po_temp a
                                where userid=? 
                                order by kodeprod
                            ";
                    ...
                    if($query->num_rows() > 0)
                    {
                        ...
                        foreach ($query->result() as $value)
                        {
                            $this->table->set_heading('Total Berat | Volume','','','','','',
                            $value->x
                            );
                        }
                ...
                case 'addtemp':
                {
                    ...
                    $post['banyak_karton']  =$this->input->post('amount');
                    $post['harga']   =$this->getHarga($kodeprod,$id);
                    $post['supp']    =$row->supp;
                    $post['grup']    =$row->grup;
                    $post['berat']  =$row->berat;
                    $post['volume']  =$row->volume;
                ...
                case 'save'://purchase
                {
                    ...
                    if ($kode_alamat == '' || $np == '' || $npwp == '' || $email == '' ){
                        echo "<script>
                        alert('Harap Perbarui Data Anda !');
                        window.location.href='../../trans/spk/confirm';
                        </script>";
                    }
                    ...
                    $id=$this->session->userdata('id');
                    $userid=$this->session->userdata('id');
                    $sql="  select company,npwp, address,email, b.alamat, b.kode_alamat from mpm.user a
                            LEFT JOIN mpm.t_alamat b on a.username = b.username 
                            WHERE id=$id and b.kode_alamat = '$kode_alamat'";
                    $row=$this->db->query($sql)->row();
                    $row2=$this->getproduct($kodeprod);
                    $post['created_by'] =$userid;
                    $post['created']=$tgl; //diubah
                    $post['tglpesan']=$tgl; //diubah
                    $post['userid']= $id;
                    $post['supp']=$this->session->userdata('supplier');
                    $post['tipe']='S';
                    $post['alamat']=$row->alamat;
                    $post['kode_alamat']=$row->kode_alamat;
                    $post['company']=$row->company;
                    $post['email']=$row->email;
                    $post['npwp']=$row->npwp;
                    $post['grup']=$this->session->userdata('group');

                    
    
                    // $tglpesan=date('Y-m-d H:i:s'); //ini srouce asal
                    $tglpesan=$tgl; // diganti menjadi ini
    
                    $this->db->trans_begin();
                    $this->db->insert('mpm.po',$post);
                    $query=$this->db->query("select id from mpm.po where created='".$post['created']."' and userid=".$id);
                    $row=$query->row();
                    $sql='insert into mpm.po_detail(id_ref,supp,kodeprod,namaprod,banyak,banyak_karton,harga,kode_prc,berat, volume, userid) select '.$row->id.',supp,kodeprod,namaprod,sum(banyak),sum(banyak_karton),harga,kode_prc, berat, volume, userid from mpm.po_temp where userid='.$id.' and supp='.$supp.' group by kodeprod';
                    $this->db->query($sql);
        ...
        switch(strtolower($state))
        {
                ...
                switch($report){
                    ...
                    case 3:
                        ...
                        $sql="select a.tglbuat,a.nopo as nott,a.npwp,a.nama_wp,a.nodo,a.tglfaktur_beli,a.noseri,a.dpp,a.ppn,(a.dpp+a.ppn)tot_ret_client,a.supp, b.npwp as npwp_supp,b.namasupp,a.nodo_beli,a.tglfaktur,a.noseri_beli,a.dpp_beli,a.ppn_beli,(a.dpp_beli+a.ppn_beli) tot_ret_supp 
                            ...
                            format(sum((abs(banyak)*harga)-(abs(banyak)*harga*diskon/100)),2) as dpp,
                            format(sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100)),2) as dpp_beli,
                            format(sum((abs(banyak)*harga)-(abs(banyak)*harga*diskon/100))/10,2) as ppn,
                            format(sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100))/10,2) as ppn_beli from  mpm.trans a
                            inner join mpm.trans_detail b on a.id=b.id_ref where a.deleted=0 and b.deleted=0 and date_format(tglbuat,'%m%Y')='".$tanggal."' group by a.id)a
                            inner join  mpm.tabsupp b using(supp)";
                        /*
                        $sql="
                        select  a.tglbuat,a.nopo as nott,
                                a.npwp,a.nama_wp,a.nodo,
                                a.tglfaktur_beli,a.noseri,
                                a.dpp,a.ppn,
                                (a.dpp+a.ppn)tot_ret_client,
                                a.supp, b.npwp as npwp_supp,
                                b.namasupp,a.nodo_beli,a.tglfaktur,
                                a.noseri_beli,a.dpp_beli,a.ppn_beli,
                                format((a.dpp_belix+a.ppn_belix),0) tot_ret_supp 
                        from 
                        (
                            select  npwp,nama_wp,a.supp,noseri,noseri_beli,
                                    nodo,nopo,nodo_beli,date_format(tgldo,'%d/%m/%Y') as tglfaktur,
                                    date_format(tgldo_beli,'%d/%m/%Y') as tglfaktur_beli,
                                    date_format(tglbuat,'%d/%m/%Y') as tglbuat,
                                    format(sum((banyak*harga)-(banyak*harga*diskon/100)),2) as dpp,
                                    format(floor(sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100))),0) as dpp_beli,
                                    format(sum((banyak*harga)-(banyak*harga*diskon/100))/10,2) as ppn,
                                    format(floor(sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100)))/10,0) as ppn_beli,
                                    sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100)) as dpp_belix,
                                    sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100))/10 as ppn_belix
                            from    mpm.trans a inner join mpm.trans_detail b 
                                            on a.id=b.id_ref 
                            where   a.deleted=0 and b.deleted=0 and date_format(tglbuat,'%m%Y')='".$tanggal."' 
                            group by a.id
                        )a
                            inner join  mpm.tabsupp b using(supp)";
                        */
                        ...
                        case 4:                        
                            $sql="
                            select 	if(a.nodo_beli = '', 'null', a.nodo_beli) as no_retur, a.noseri_beli as no_faktur_pajak, a.tgldo, a.company,
                                    b.kodeprod, b.namaprod, abs(b.banyak) as qty, b.harga_beli, (abs(b.banyak) * b.harga_beli) as harga_jual_bkp, format((abs(b.banyak) * b.harga_beli),2)  as harga_jual_bkp_1
                            from mpm.trans a inner JOIN mpm.trans_detail b
                                        on a.id = b.id_ref
                            where date_format(tglbuat,'%m%Y')='".$tanggal."' and a.supp = 005 and a.deleted = 0
                            ORDER BY a.tglbuat desc, a.company asc
                            ";                       
                            return to_excel($this->db->query($sql),''.$tanggal);
                            break;
                            // echo "<pre>";
                            // print_r($sql);
                            // echo "</pre>";
                        ...
                        case 5:
                            $sql = "
                            select 	a.tglbuat,a.nopo as nott,a.npwp,a.nama_wp,a.nodo,a.tglfaktur_beli,
                                    a.noseri,a.dpp,a.ppn,(a.dpp+a.ppn)tot_ret_client,a.supp, 
                                    b.npwp as npwp_supp,b.namasupp,a.nodo_beli,a.tglfaktur,
                                    a.noseri_beli,
                                    format(a.dpp_beli,0) as dpp_beli,
                                    /*format(a.ppn_beli,0) as ppn_beli,*/
                                    a.ppn_beli,
                                    format((a.dpp_beli+a.ppn_beli),0) as tot_ret_supp 
                            from 
                            (
                                select 	npwp,nama_wp,a.supp,noseri,noseri_beli,nodo,nopo,
                                        nodo_beli,date_format(tgldo,'%d/%m/%Y') as tglfaktur,
                                        date_format(tgldo_beli,'%d/%m/%Y') as tglfaktur_beli,
                                        date_format(tglbuat,'%d/%m/%Y') as tglbuat,
                                        format(sum((abs(banyak)*harga)-(abs(banyak)*harga*diskon/100)),2) as dpp,
                                        /*format(sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100)),0) as dpp_beli,*/
                                        sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100)) as dpp_beli,
                                        format(sum((abs(banyak)*harga)-(abs(banyak)*harga*diskon/100))/10,2) as ppn,
                                        /*format(sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100))/10,0) as ppn_beli */
                                        sum((banyak*harga_beli)-(banyak*harga_beli*diskon_beli/100))/10 as ppn_beli 
                                from  	mpm.trans a inner join mpm.trans_detail b 
                                            on a.id=b.id_ref 
                                where 	a.deleted=0 and b.deleted=0 and date_format(tglbuat,'%m%Y')='".$tanggal."' and a.supp = 001
                                group by a.id
            ...
            case 'print_beli_marguna':
                {
                    ...
                    $db='mpm';
                    $this->load->library('PHPJasperXML');
                    echo$xml = simplexml_load_file("assets/report/report_retur_supp_marguna.jrxml");
                    @$this->phpjasperxml->debugsql=false;
                    @$this->phpjasperxml->arrayParameter=array('nopesan'=>$key);
                    @$this->phpjasperxml->xml_dismantle($xml);
                ...
                case 'print_beli_intrafood':
                    {
                        ...
                        $db='mpm';
                        $this->load->library('PHPJasperXML');
                        echo$xml = simplexml_load_file("assets/report/report_retur_supp_intrafood.jrxml");
                        @$this->phpjasperxml->debugsql=false;
                        @$this->phpjasperxml->arrayParameter=array('nopesan'=>$key);
                        @$this->phpjasperxml->xml_dismantle($xml);
            ...
            case 'print_beli_us':
                {
                    ...
                    $db='mpm';
                    $this->load->library('PHPJasperXML');
                    echo$xml = simplexml_load_file("assets/report/report_retur_supp_us.jrxml");
                    @$this->phpjasperxml->debugsql=false;
                    @$this->phpjasperxml->arrayParameter=array('nopesan'=>$key);
                    @$this->phpjasperxml->xml_dismantle($xml);
                ...
                case 'print_beli_deltomed':
                    {
                        ...
                        $db='mpm';
                        $this->load->library('PHPJasperXML');
                        echo$xml = simplexml_load_file("assets/report/report_retur_supp_deltomed.jrxml");
                        @$this->phpjasperxml->debugsql=false;
                        @$this->phpjasperxml->arrayParameter=array('nopesan'=>$key);
                        @$this->phpjasperxml->xml_dismantle($xml);
            ...
            case 'show':
            {
                ...
                switch($id)
                {
                     ...
                     {
                          $sql="
                            select  a.id,a.supp,a.userid,a.company,a.tipe,
                                    a.deleted,tglbuat,a.created,a.nodo_beli,
                                    nodo, 
                                    date_format(tglbuat,'%d %M %Y, %T') as tglbuat,
                                    date_format(a.tgldo,'%Y-%m-%d') as tgldo,
                                    noseri
            ...
            case 'update':
            {
                ...
                $post['tgldo']=$this->input->post('tgldo_beli');
            ...
            case 'save'://purchase
            {
                ...
                $row=$this->getCustInfoRetur($client);
                $post['created_by'] =$userid;
                $post['created']=date('Y-m-d H:i:s');
                ...
                $post['tipe']='R';
                $post['company']=$row->company;
                $post['nama_wp']=$row->nama_retur;
                $post['email']=$row->email;
                $post['npwp']=$row->npwp_retur;
                $post['alamat_wp']=$row->alamat_wp;
                $post['tgl_beli']=$this->input->post('tgl_beli');
                ...
                $row=$query->row();
                $sql='
                insert into mpm.trans_detail(id_ref,supp,kodeprod,namaprod,banyak,harga,harga_beli,kode_prc,userid,diskon,diskon_beli,created_by,created) 
                select  '.$row->id.',supp,
                        kodeprod,namaprod,
                        sum(banyak)*(-1),
                        harga,harga_beli,
                        kode_prc,'.$post['userid'].',
                        diskon,diskon_beli,
                        '.$id.',"'.$post['created'].'" 
                from    mpm.trans_temp 
                where   userid='.$id.' and supp="'.$supp.'" group by kodeprod';
                $this->db->query($sql);
                ...
                //echo "<br>status trans_complete : ".$this->db->trans_status() ; 
        ...
        switch($state)
        {
                ...
                switch($id)
                {
                     case '0':
                     {
                        ...
                        (
                            ...
                            from
                            (
                                    select  a.open,a.open_by,c.username,a.open_date,a.id,a.supp,a.company,
                                            b.id as userid,
                                            b.kode_lang,a.email,tipe,
                                            date_format(tglpesan,'%d %M %Y, %T') as tglpesan,
                                            nopo, date_format(tglpo,'%d %M %Y, %T') as tglpo,
                                            nodo, date_format(tgldo,'%d %M %Y, %T') as tgldo,
                                            b.bank_garansi
                            ...
                            left join 
                            (
                                select  id_ref,sum(harga*banyak) as value 
                                from    po_detail 
                                where   deleted=0 
                                group by id_ref
                            )b on a.id=b.id_ref
                        ...
                        (
                            ...
                            (
                                ...
                                where   dokument-bayar>0 group by customerid
                            ) a
                            inner join 
                            (
                                select  substr(customerid,2,5) kode_lang ,
                                        ...
                                        from    (
                                                ...
                                                (       
                                                    select  id_ref,
                                                            ...
                                                            sum(berat*banyak_karton) as subberat
                                                            from mpm.po_detail 
                                                    where   deleted=0 
                                                    group by id_ref
                                                )b  on a.id=b.id_ref                
                    ...
                    if($query->num_rows()>0)
                        {
                            ...
                            $this->table->set_template($this->tmpl);
                            $this->table->set_caption('ORDER LIST');
                            $this->table->set_heading('NO ORDER','COMPANY','ORDER DATE', 'SUPPLIER','TIPE','NO PO','ORDER VALUE','PRINT','PIUTANG','BANK GARANSI','CREDIT LIMIT','DUEDATE (>7)','LOCK/UNLOCK','OPEN BY');
                            ...
                            foreach ($query->result() as $value)
                            {
                                ...
                                $this->table->add_row(
                                        ...
                                        //,'<div style="text-align:center">'.anchor('trans/po/show_detail/'.$value->id.'/'.$value->supp,img($image_properties)).'</div>'
                                        ,$detail
                                        ,'<div style="text-align:right">'.($value->username).'</div>'
                                        );
                                    }
            ...
            case 'rekap':
            {
                ...
                $sql="
                    select  a.company, 
                            ...
                            b.harga,
                            b.banyak_karton,
                            b.banyak*b.harga as total,
                            b.banyak_karton*b.berat as subberat,
                            tglpo,nopo 
                    from    mpm.po a inner join  mpm.po_detail b 
                                on a.id=b.id_ref  
                    where   a.deleted=0 and b.deleted=0 and b.supp=".$supp."  and 
                            month(tglpo)=".$month." and 
                            year(tglpo)=".$year." 
                    order by nopo,kodeprod";
                /*
                echo "<pre>";
                print_r($sql);
                echo "</pre>";
            ...
            case 'show':
            {
                            ...
                            (
                                    ...
                                    (
                                        select  a.open,
                                                ...
                                                date_format(tglpesan,'%d %M %Y, %T') as tglpesan,
                                                nopo, date_format(tglpo,'%d %M %Y, %T') as tglpo,
                                                nodo, date_format(tgldo,'%d %M %Y, %T') as tgldo, note_acc
                                        from    mpm.po a 
                ...
                if($query->num_rows()>0)
                {
                    ...
                    $this->table->set_template($this->tmpl);
                    $this->table->set_caption('ORDER LIST');
                    $this->table->set_heading('DELETE','NO ORDER','COMPANY','ORDER DATE', 'NO PO','PO DATE','SUPPLIER','TIPE','TOTAL VALUE','DETAIL','PRINT','EMAIL', 'NOTE ACC','CSV','US(rev)');
                    ...
                    foreach ($query->result() as $value)
                    {
                        ...
                        $image_email_warn = ($value->jt>0 and $value->email!='') ?
                                '<div div style="text-align:center">'.anchor('trans/po/email_warn/'.$value->id.'/'.$value->userid.$value->supp,img($this->image_properties_email)).'</div>' :
                                '<div div style="text-align:center">'.img($this->image_properties_email_off).'</div>';
                        $image_email_csv = ($value->tglpo!='' and $value->email!='') ?
                                '<div div style="text-align:center">'.anchor('all_po/export_csv/'.$value->id,img($this->image_properties_email)).'</div>' :
                                '<div div style="text-align:center">'.img($this->image_properties_email_off).'</div>';
                        $us = ($value->tglpo!='' and $value->email!='') ?
                                '<div div style="text-align:center">'.anchor('all_po/export_po_us/'.$value->id,img($this->image_properties_email),'target="_blank"').'</div>' :
                                '<div div style="text-align:center">'.img($this->image_properties_email_off).'</div>';
                        $us_rev = ($value->tglpo!='' and $value->email!='') ?
                                '<div div style="text-align:center">'.anchor('all_po/export_po_us_r/'.$value->id,img($this->image_properties_email),'target="_blank"').'</div>' :
                                '<div div style="text-align:center">'.img($this->image_properties_email_off).'</div>';
                        ...
                        $this->table->add_row(
                                ...
                                //,'<div div style="text-align:center">'.anchor('trans/po_delete/'.$value->nopesan,img($image_properties_del),$js).'</div>'
                                //,$image_email_warn
                                ,$value->note_acc, $image_email_csv, $us_rev
                                );
                    }
            ...
            case 'email':
            {
                ...
                //echo "TIPE PO : ".$this->getTipe2($key);
               
                if($this->getTipe2($key)=='B')
                {
                    //$this->email->from('muliaputramandiri@gmail.com', '('.$this->getTipe($key).') '.'Automatic Email Sender');
                    $this->email->from('tria@muliaputramandiri.com', 'PO Kebutuhan Stok '.date('F Y'));
                }
                else
                {
                    $this->email->from('agustriatriani@gmail.com', '('.$this->getTipe($key).') '.'Automatic Email Sender');
                }

                //$file = fopen(APPPATH . '/../assets/po/'.$filename.'.csv', 'wb');
                ...
                echo "id : ".$id."<br>";
                echo "id : ".substr($id,-6,3)."<br>";
                //echo "panjang karakter : ".strlen($id)."<br>";
                if (strlen($id) == 5) {
                    $jawa = $this->getJawa(substr($id,-6,2)); 
                } else {
                    $jawa = $this->getJawa(substr($id,-6,3)); 
                }
                ...
                if ($jawa == '0') {
                    echo "<br><br>";
                    echo "Ini DP Luar Jawa<br><br>*******************<br><br>";
                    $harga_beli_candy = 'h_beli_mpm_candy_Ljawa';
                }else{
                    echo "";
                    echo "Ini DP Jawa<br><br>";
                    $harga_beli_candy = 'h_beli_mpm_candy_jawa';
                }
                echo "level : ".$this->getLevel(substr($id,-6,3))."<br>";
                $level = $this->getLevel(substr($id,-6,3));                  

                if ($level == '4') {
                    $harga_beli = 'h_beli_mpm';
                }elseif ($level == '6') {
                    $harga_beli = 'h_beli_mpm_bsp';
                }elseif ($level == '7') {
                    $harga_beli = 'h_beli_mpm_batam';
                }else{
                    $harga_beli = 'h_beli_mpm';
                }
                ...
                $sql = "
                    select  a.nopo, d.customerid, c.company, d.alamat,  c.address as ship_to,  
                            b.kodeprod, e.dl_kodeprod, e.dl_description, b.namaprod, banyak, f.$harga_beli as harga_beli_x,
                            if(a.grup = 'G0101',h.d1,if(a.grup = 'G0102',h.d2, if(a.grup = 'G0103',h.d3,''))) as kode_dp
                    ...
                    LEFT JOIN mpm.prod_detail f
                        on b.kodeprod = f.kodeprod
                    LEFT JOIN mpm.tabprod g
                        on b.kodeprod = g.KODEPROD LEFT JOIN
                    (
                        select a.kode,a.d1,a.d2,a.d3, left(a.kode,3) as kode_dp
                        from db_po.t_mapping_kode_po_deltomed a
                    )h on c.username = h.kode_dp
                    where a.id = '$key' and a.deleted = 0 and b.deleted = 0 and 
                        f.tgl = (select max(tgl) from mpm.prod_detail g where f.kodeprod = g.kodeprod) and g.grup in ('g0101','g0103')
                    
                    union all

                    select  a.nopo, d.customerid, c.company, d.alamat,  c.address as ship_to,  
                            b.kodeprod, e.dl_kodeprod, e.dl_description, b.namaprod, banyak, f.$harga_beli_candy as harga_beli_x,
                            if(a.grup = 'G0101',h.d1,if(a.grup = 'G0102',h.d2, if(a.grup = 'G0103',h.d3,''))) as kode_dp
                    from    mpm.po a LEFT JOIN
                    (
                        select  id_ref,kodeprod, namaprod, banyak, deleted
                    ...
                    where a.id = '$key' and a.deleted = 0 and b.deleted = 0 and 
                        f.tgl = (select max(tgl) from mpm.prod_detail g where f.kodeprod = g.kodeprod) and g.grup = 'g0102'
                    ORDER BY KODEPROD
                ";
                //suffyyanuar
                // echo "<pre>";
                ...
                foreach ($hasil->result() as $row) 
                {
                    ...
                    $dl_kodeprod = $row->dl_kodeprod;
                    $dl_description = $row->dl_description;
                    $namaprod = $row->namaprod;
                    $banyak = $row->banyak;
                    $h_beli_mpm = $row->harga_beli_x;
                    $kode_dp = $row->kode_dp;
                    ...
                    <td>$h_beli_mpm</td>
                    <td>$kode_dp</td>";
                    fputcsv($file, array($nopo,$customerid,$company,$alamat,$ship_to,$kodeprod,$dl_kodeprod,$dl_description,$namaprod, $banyak, $h_beli_mpm, $kode_dp));
                }
                ...
                if($cekSupp == '002')
                {
                    ...
                    $lista = $this->getEmail(substr($id,0,-3)).","."windaamelia078@gmail.com,tria@muliaputramandiri.com";
                ...
                elseif($cekSupp == '005'){
                    ...
                    $listsupp = $this->getEmailSupp(substr($id,-3)).","."netty.herawati@ultrasakti.com";    
                    $hasilto = explode(',', $listsupp);

                    $lista = $this->getEmail(substr($id,0,-3)).","."tria@muliaputramandiri.com";
                    // $lista = $this->getEmail(substr($id,0,-3)).","."netty.herawati@ultrasakti.com";
                    $hasil = explode(',', $lista);

                    $this->email->to($hasilto);
                    $this->email->cc($hasil);

                    echo "<pre>";
                    echo "email to principal : ".$listsupp."<br>";
                    // echo "hasilto : ".$hasilto."<br>";
                    echo "email cc to dp dan rm am : ".$lista."<br>";
                    // echo "hasil : ".$hasil."<br>";
                    echo "</pre>";

                    //$this->email->to('suffy.yanuar@gmail.com');
                    //$this->email->cc($hasil);xxxx

                    $this->email->subject('PO NO '.$filename);
                    $this->email->message('This email is sent by system');
                    $this->email->attach('assets/po/'.$filename.'.pdf');

                    $proses = $this->email->send();
                    if($proses == 1){
                        echo "email terkirim";
                        redirect('all_po/export_po_us/'.$key,'refresh');
                    }else{
                ...
                elseif($cekSupp == '001'){
                    ...
                    $listsupp = $this->getEmailSupp(substr($id,-3));   
                    $hasilto = explode(',', $listsupp);

                    $lista = $this->getEmail(substr($id,0,-3)).","."dian.nugraha@deltomed.com";
                    ...
                    $supp = substr($id,-3);
                    $dp = substr($id,0,-3);
                    echo anchor("all_po/kirimEmailPo/$supp/$dp/$filename", 'KIRIM EMAIL',"class='btn btn-primary btn-sm'");
                ...
                }elseif($cekSupp == '012')
                {
                    ...
                    $listsupp = $this->getEmailSupp(substr($id,0,-3));
                    $lista = $this->getEmail(substr($id,0,-3)).","."tria@muliaputramandiri.com, suffy@muliaputramandiri.com";

                    $hasil = explode(',', $lista);
                    $this->email->cc($hasil);

                    // $this->email->to("suffy@muliaputramandiri.com, suffy.yanuar@gmail.com");
                    // $this->email->cc("tria@muliaputramandiri.com");
            ...
            case 'manual_save':
            {
                ...
                if ($kode_alamat == '' || $np == '' || $npwp == '' || $email == '' ){
                    echo "<script>
                    alert('Harap Perbarui Data Anda !');
                    window.location.href='../../trans/po/manual_confirm'';
                    </script>";
                }
                ...
                $sql='insert into mpm.po_detail(id_ref,supp,kodeprod,namaprod,banyak,harga,banyak_karton,kode_prc,berat,volume,userid) select '.$row->id.',supp,kodeprod,namaprod,sum(banyak),harga,sum(banyak_karton),kode_prc, berat,volume,userid from mpm.po_temp where userid='.$userid.' and supp='.$supp.' group by kodeprod';
            ...
            case 'show_temp':
            {
                ...
                // "ilham"
                $sql1='
                    select a.id,a.id_ref,a.supp,a.kodeprod,a.namaprod,a.banyak,
                            a.banyak_karton,a.harga,a.kode_prc,a.userid,a.berat, a.volume, b.isisatuan  
                    from     mpm.po_temp a LEFT JOIN mpm.tabprod b
                ...
                if($query->num_rows() > 0)
                {
                    ...
                    $this->table->set_template($this->tmpl);
                    $this->table->set_caption('PURCHASE ORDER');
                    $this->table->set_heading('CODE','PRODUCT', 'PRC CODE','AMOUNT','BERAT','VOLUME (m3)','DELETE');
                    foreach ($query->result() as $value)
                    {
                        $js=array('onclick'=>"return confirm('Are you sure you want to delete?')");
                        $this->table->add_row(
            ...
            {
                $sql="
                select a.userid, a.supp, a.nopo, a.tglpo, a.nodo, a.tgldo, a.tglpesan,
                     a.created, a.modified, a.created_by, a.modified_by,
                     a.tipe, a.`open`, a.open_by, a.open_date, a.company, a.npwp, a.email, 
                     a.alamat, a.deleted, a.ambil, a.note,
                     b.id, b.id_ref, b.supp, b.kodeprod, b.namaprod, b.banyak, b.`backup`,
                     b.harga, b.banyak_karton, b.kode_prc, b.berat, b.userid, b.deleted,
                     b.stock_akhir, b.rata3bulan, (b.banyak + b.stock_akhir)/rata3bulan as doi 
                from mpm.po a inner join mpm.po_detail b 
                        on a.id=b.id_ref 
                where a.id=? and b.deleted=0 and a.deleted=0 
                order by kodeprod";
                ...
                if($query->num_rows()>0)
                {
                    ...
                    $this->table->set_template($this->tmpl);
                    $this->table->set_caption('PURCHASE ORDER DETAIL');
                    $this->table->set_heading('CODE','PRODUCT', 'PRC CODE','AMOUNT','STOCK','AVG3BULAN','DOI','DELETE');
                    foreach ($query->result() as $value)
                    {
                        if ($value->doi >= 8) {
                             ...
                             $js=array('onclick'=>"return confirm('Are you sure you want to delete?')");
                             $this->table->add_row(
                                '<div div style="font-weight:bold;color:red;text-align:right">'.
                                $value->kodeprod.'</div>',
                                '<div div style="font-weight:bold;color:red;text-align:right">'.
                                $value->namaprod.'</div>',
                                '<div div style="font-weight:bold;color:red;text-align:right">'.
                                $value->kode_prc.'</div>',
                                '<div div style="font-weight:bold;color:red;text-align:right">'.number_format($value->banyak).'</div>',
                                '<div div style="color:red;text-align:right">'.number_format($value->stock_akhir).'</div>',
                                '<div div style="color:red;text-align:right">'.$value->rata3bulan.'</div>',
                                '<div 
                                    div style="color:red;text-align:right">'

                                .number_format($value->doi).'</div>',
                             ...
                             $js=array('onclick'=>"return confirm('Are you sure you want to delete?')");
                             $this->table->add_row(
                                '<div div style="font-weight:bold;color:orange;text-align:right">'.
                                $value->kodeprod.'</div>',
                                '<div div style="font-weight:bold;color:orange;text-align:right">'.
                                $value->namaprod.'</div>',
                                '<div div style="font-weight:bold;color:orange;text-align:right">'.
                                $value->kode_prc.'</div>',
                                '<div div style="font-weight:bold;color:orange;text-align:right">'.number_format($value->banyak).'</div>',
                                '<div div style="color:orange;text-align:right">'.number_format($value->stock_akhir).'</div>',
                                '<div div style="color:orange;text-align:right">'.$value->rata3bulan.'</div>',
                                '<div 
                            ...
                            $this->table->add_row(
                                    ...
                                    $value->kode_prc,
                                    '<div div style="text-align:right">'.number_format($value->banyak).'</div>',
                                    '<div div style="text-align:right">'.number_format($value->stock_akhir).'</div>',
                                    '<div div style="text-align:right">'.$value->rata3bulan.'</div>',
                                    '<div 
    ...
    {
        $sql='select nopo,note,alamat, company from mpm.po where id=?';
        $query=$this->db->query($sql,array($id));
        return $query->row();
    }
    ...
    public function getEmailSupp($supp)
    {
    ...
    {
        $sql='select level from mpm.user where id=?';
        $query=$this->db->query($sql,array($id));
        $row=$query->row();
        return $row->level;
    }

    public function getJawa($id)
    {        

        $sql='select jawa
        from mpm.tbl_tabcomp a LEFT JOIN
        mpm.`user` b on a.kode_comp = b.username 
        where b.id = ?
        ';
        $query=$this->db->query($sql,array($id));
        $row=$query->row();
    ...
    public function list_client_repl($userid=0)
    {
        $sql="
            select  distinct b.naper as nocab,
                    a.id,kode_comp,nama_comp,b.company_repl company
            from    mpm.user a inner join mpm.tabcomp b 
                        on b.kode_comp=a.username 
            order by nama_comp
        ";
        return $this->db->query($sql,array($userid));
        //return $query->row();
    ...
    {
        /* note : sebelumnya menampilkan level 4,5,6,7 */
        $sql="
            select  id,
                    kode_dp, username, 
                    company 
    ...
    public function list_client_po()
    {
        $id=$this->session->userdata('id');
        /* note : sebelumnya menampilkan level 4,5,6,7 */
        if ($id == 4420 || $id == 588) {
    ...
    public function list_client_retur($userid=0)
    {
        /* note : sebelumnya menampilkan level 4,5,6,7 */
        $sql="
            select  id,
    ...
    public function cek_supp(){
        $id_retur = $this->uri->segment('4');
        $sql = $this->db->query("select supp from mpm.trans where id = $id_retur")->result();
        foreach($sql as $x){
            return $x->supp;
        }
    }
