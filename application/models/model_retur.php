...
class Model_retur extends CI_Model 
{
    ...
    public function generate($data){
        ...
        if ($proses) 
        {
            ...
            from mpm.trans a INNER JOIN mpm.trans_detail b
                ...
                on a.supp = c.supp
            where a.tgl_beli BETWEEN '$periode_1' and '$periode_2' and a.deleted = 0 and b.deleted = 0
    ...
    public function get_site_code_by_signature($signature){
        $query = "select left(site_code,3) as site_code from db_temp.t_temp_pengajuan_retur a where a.signature = '$signature'";
    ...
    public function history_pengajuan_retur($kode_alamat, $id = null, $from = '', $to = '')
    {
        if ($id == null) {
            $params = "";
        }else{
        ...
        if ($from && $to) {
            $params_time = "and a.tanggal_pengajuan BETWEEN '$from' and '$to'";
            $params_limit = "";
        }else{
            $params_time = "";
            $params_limit = "limit 10000";
        }
        $sql = "
        select a.id, a.no_pengajuan, a.site_code, a.nama, a.supp, a.tanggal_pengajuan, a.`status`, a.signature, 
        if(a.supp ='001-herbal', 'DELTOMED-HERBAL',if(a.supp='001-herbana','DELTOMED-HERBANA',c.namasupp)) as namasupp, a.nama_status, if(b.nama_comp is null, d.company, b.nama_comp) as nama_comp
        ...
        )b on a.site_code = b.kode left join mpm.tabsupp c on a.supp = c.supp left join 
        (
            select a.id, a.username, a.company
            from mpm.user a 
        )d on a.site_code = d.username
        where a.deleted is null and a.site_code in ($kode_alamat) $params $params_time order by id desc $params_limit
    ...
    public function log_retur($signature)
    {
        ...
        $sql = "
            select  a.*, if(b.branch_name is null, d.name, b.branch_name) as branch_name, 
                    if(b.nama_comp is null, d.company, b.nama_comp) as nama_comp, 
                    if(a.supp ='001-herbal', 'DELTOMED-HERBAL',
                    if(a.supp='001-herbana','DELTOMED-HERBANA',c.namasupp)) as namasupp
            from db_temp.t_temp_pengajuan_retur a LEFT JOIN 
            (
                select concat(a.kode_comp, a.nocab) as kode, a.branch_name, a.nama_comp
                from mpm.tbl_tabcomp a
                ...
                select a.username, a.company, a.name
                from mpm.user a
            )d on a.site_code = d.username
    ...
    public function get_produk_pengajuan($id, $supp)
    {
        if ($supp == '005') {
            ...
            $query = "
            select  a.id, a.id_pengajuan, a.kodeprod, a.namaprod, a.batch_number, a.expired_date, a.jumlah, 
                    a.alasan, a.satuan, a.nama_outlet, a.keterangan, a.`status`, a.nama_status, a.deskripsi, 
                    b.no_pengajuan, b.tanggal_pengajuan, b.signature, a.total_karton, a.total_dus, a.total_pcs, 
                    a.harga_karton, a.harga_dus, a.harga_pcs, a.value, a.kode_produksi
            from    db_temp.t_temp_produk_pengajuan_retur a left join db_temp.t_temp_pengajuan_retur b 
                        on a.id_pengajuan = b.id 
            where   a.id_pengajuan = $id and a.deleted is null
            ";

        }else if ($supp == '001-herbal' || $supp == '001-herbana') 
        {
            $query = "
            select a.id, a.id_pengajuan, a.kodeprod, a.namaprod, a.batch_number, 
					a.expired_date, a.jumlah, a.alasan, a.satuan, a.nama_outlet, a.keterangan, 
					a.`status`, a.nama_status, a.deskripsi, b.no_pengajuan, b.tanggal_pengajuan, 
					b.signature, a.total_karton, a.total_dus, a.total_pcs, a.harga_karton, a.harga_dus, 
					a.harga_pcs, a.value, a.kode_produksi
            from 	db_temp.t_temp_produk_pengajuan_retur a left join db_temp.t_temp_pengajuan_retur b 
                        on a.id_pengajuan = b.id 
            where a.id_pengajuan = $id and a.deleted is null
            ";
    ...
    public function generate_attachment_pengajuan_retur($id,$supp){
        ...
        if ($supp == '002') {
            ...
            foreach ($hasil->result() as $row) 
            {
                ...
                $nama_status = $row->nama_status;
                $deskripsi = $row->deskripsi;
                fputcsv($file, array($no_pengajuan,$kodeprod,$namaprod,$batch_number,$satuan,$expired_date,$jumlah,$nama_outlet,$alasan,$keterangan,$nama_status,$deskripsi));
            }
    ...
    public function get_pengajuan_retur($id){
        ...
        select 	a.no_pengajuan, a.site_code, a.nama, a.supp, a.tanggal_pengajuan, 
                a.file, a.status, a.created_date, a.created_by, b.branch_name, b.nama_comp,if(a.supp ='001-herbal', 'DELTOMED-HERBAL',if(a.supp='001-herbana','DELTOMED-HERBANA',c.namasupp)) as namasupp,
                ...
                a.tanggal_pemusnahan, a.nama_pemusnahan, a.file_pemusnahan, foto_pemusnahan_1, foto_pemusnahan_2, a.note, d.username
        from 	db_temp.t_temp_pengajuan_retur a LEFT JOIN
        (
            select concat(a.kode_comp, a.nocab) as kode, a.branch_name, a.nama_comp
            from mpm.tbl_tabcomp a
    ...
    public function get_pengajuan_retur_mpi($id){
        $sql = "
        select 	a.no_pengajuan, a.site_code, a.nama, a.supp, a.tanggal_pengajuan, 
                a.file, a.status, a.created_date, a.created_by, 'MPI'  as branch_name, d.company as nama_comp,
                if(a.supp ='001-herbal', 'DELTOMED-HERBAL',if(a.supp='001-herbana','DELTOMED-HERBANA',c.namasupp)) as namasupp,
                d.company, a.file_principal, a.keterangan_principal, a.file_pengiriman, a.tanggal_kirim_barang, a.nama_ekspedisi, 
                a.est_tanggal_tiba, a.tanggal_tiba, a.tanggal_approval, a.nama_status, a.file_terima, a.nama_penerima, a.no_terima, 
                a.file_terima, a.tanggal_terima,
                a.tanggal_pemusnahan, a.nama_pemusnahan, a.file_pemusnahan, foto_pemusnahan_1, foto_pemusnahan_2, a.note, d.username
        from 	db_temp.t_temp_pengajuan_retur a LEFT JOIN mpm.tabsupp c 
        on a.supp = c.supp  LEFT JOIN 
        (
            select a.username, a.company
            from mpm.user a
    ...
    public function get_email_dp_mpi($site_code){

        $sql = "
            select a.email
            from mpm.`user` a
            where a.username = '$site_code'
        ";
        $proses = $this->db->query($sql)->result();
        if ($proses) {
            return $proses;
    ...
    public function get_email_principal_mpi($supp){

        // kalau herbal maka ditujukan ke head sales mt -> pak nudy
        if ($supp == '001-herbal') {
            $query = "
                select a.email_mt as email
                from site.map_email_retur a
                where a.supp = '$supp'
            ";
    ...
    public function get_status_pengajuan($id){
        $sql = "
            select a.no_pengajuan, a.status, a.nama_status, b.branch_name, if(b.nama_comp is null, c.company, b.nama_comp) as nama_comp, a.signature, a.supp, a.note
            from db_temp.t_temp_pengajuan_retur a
            left join
            (
                select concat(a.kode_comp, a.nocab) as kode, a.branch_name, a.nama_comp
                ...
                where a.status = 1
                GROUP BY concat(a.kode_comp, a.nocab)
            )b on a.site_code = b.kode LEFT JOIN 
            (
                select a.username, a.company
                from mpm.user a 
            )c on a.site_code = c.username          
            where a.id = $id
        ";
        $proses = $this->db->query($sql)->result();
        if ($proses) {
    ...
    public function get_tableimport_product_deltomed()
    {
        ...
        $signature = $this->uri->segment(3);
        $sql = "
            select a.id, a.category, a.id_pengajuan, a.kodeprod, a.batch_number, a.expired_date, sum(a.jumlah) as jumlah, a.alasan, a.satuan, a.nama_outlet, a.keterangan, b.namaprod
            from db_temp.t_temp_pengajuan_retur_import a
            LEFT JOIN mpm.tabprod b on a.kodeprod = b.kodeprod
    ...
    public function delete_history_import($signature,$supp){
        $sql = "
            delete from db_temp.t_temp_pengajuan_retur_import
            where signature = '$signature'
        ";
        $deleted = $this->db->query($sql);
        if ($deleted) {
            redirect("retur/pengajuan/$signature/$supp");
        }
    }

    public function get_header_retur($periode_1 = '', $periode_2 = ''){

        if ($periode_1 == '') {
            $periode_1 = '1900-01-01';
        }

        if ($periode_2 == '') {
            $periode_2 = '1900-01-01';
        }
    ...
    public function get_alasan(){
        $query = "
            select a.alasan, a.signature, a.created_at, b.username
            from site.t_retur_alasan a left join 
            (
