...
class Model_management_rpd extends CI_Model 
{
    public function get_pengajuan($id = ""){
        ...
        if ($username == 'suffy' || $username == 'ratri'  || $username == 'nanita' || $username == 'hendra') {
            $params_admin = "";
            $params = "";
        }elseif ($username == 'evlin'|| $username == 'anto') {
            $params_admin = "where  (c.supp = '001')"; 
                if ($id) {
                    $params = "and a.id = $id";
                }else{
                    $params = "";
    ...
    public function get_pengajuan_bysignature($signature = ""){
        ...
        $query = "
            select 	a.id, a.no_rpd, a.pelaksana, a.jabatan, a.tanggal_mulai, a.tanggal_akhir, a.maksud_perjalanan_dinas, a.tanggal_berangkat, a.tempat_berangkat, 
