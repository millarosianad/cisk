...
class Trans extends MY_Controller
{
    ...
    function retur($state='show',$key=0,$id=0,$init=0,$userid=0)
    {
        ...
        switch($state)
        {
            ...
            case 'show':
            {
                ...
                $this->load->library('pagination');
                $limit = 10;
                $data['query2']=$this->tmodel->list_client_retur();
            ...
            case 'print_beli':
            {
                $supp = $this->tmodel->cek_supp($key);

                if ($supp == '005') {
                    $this->tmodel->retur('print_beli_us',$key);
                }elseif ($supp == '001') {
                    $this->tmodel->retur('print_beli_deltomed',$key);
                }elseif ($supp == '002') {
                    $this->tmodel->retur('print_beli_marguna',$key);
                }elseif ($supp == '012') {
                    $this->tmodel->retur('print_beli_intrafood',$key);
                }else{
            ...
            case 'add':
            {
                ...
                //echo "<br><br><br><br><br><br><br><br><br>c";
                if($produk!='')
                {
                    //echo "<br><br><br><br><br><br><br><br>a";
    ...
    function repl($state='null',$key=null,$id=null,$init=null,$offset=null)
    {
        ...
        switch(strtolower($state))
        {
            ...
            case 'list_upload':
            {
                ...
                $data['query2']=$this->tmodel->list_client_repl();
    ...
    function po($state='null',$key=null,$id=null,$init=null,$offset=null)
    {
        ...
        switch(strtolower($state))
        {
            ...
            case 'show_supp':
            {
                ...
                $data['query']=$this->tmodel->getSupp();
                $data['query2']=$this->tmodel->list_client_po();
            ...
            case 'set_supp':
            {
                ...
                if($client == 0){
                    redirect(site_url('trans/po/show_supp'));
                    }
                $sql = "
                select min_berat
                ...
                foreach ($proses as $key) {
                    $min_berat = $key->min_berat;
                }
                $newdata = array(
                   'supplier'  => $supp,
                   'group'  => $group,
                   'client'  => $client,
                   'min_berat_po'=>$min_berat
                );
                $this->session->set_userdata($newdata);
            ...
            case 'manual':
            {
                ...
                $data['table']=$this->tmodel->po('show_temp_total');
                $data['query2']=$this->tmodel->getBerat($id);

                // echo "<br><br><br><br>xxxxxxxxxxxxxxxxx";
                // var_dump($this->tmodel->getBerat($id));
            ...
            case 'manual_confirm':
            {
                ...
                redirect(base_url().'all_po/konfirmasi_alamat_po',$data);
            }break;
            case 'manual_save':
            {
            ...
            case 'print':
            {
                ...
                $supp = $this->model_po->cek_supp($key);

                if ($supp == '012') {
                    $this->tmodel->po('print_intrafood',$key);
                }else{
    ...
    function spk($state='show_supp',$key=null,$id=null)
    {
        ...
        switch(strtolower($state))
        {
            ...
            case 'add':
            {
                ...
                $data['table']=$this->tmodel->spk('show_temp_total');
                $data['query2']=$this->tmodel->getBerat($id);
            }break;
            case 'confirm':
            {
                ...
                redirect(base_url().'all_po/konfirmasi_alamat_spk',$data);
            }break;
            case 'addtemp':
            {
    ...
    function buildgroup_target()
    {
        ...
        if($kode_supp == 001){
            $query=$this->tmodel->get_group($data);
